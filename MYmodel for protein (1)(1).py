...
def load_or_create_go_slim_map(original_go_annotations_dict):
    # 检查缓存文件是否存在
    cache_file = "go_to_targetslim_map.pkl"
    if os.path.exists(cache_file):
        # 如果缓存文件存在，加载缓存的映射
        with open(cache_file, 'rb') as f:
            goto_slim_map = pickle.load(f)
        print("加载缓存的GO target映射")
    else:
        # 如果缓存文件不存在，构建新的映射
        go_dag = GODag("go-basic.obo")
        goto_slim_map = map_go_to_target_classes(original_go_annotations_dict, target_classes_dict=TARGET_MF_CLASSES, obo_dag=go_dag)
        
        # 保存映射到缓存文件
        with open(cache_file, 'wb') as f:
            pickle.dump(goto_slim_map, f)
        print("构建并保存GO target映射到缓存")
...
'''
class ProtBERTFeatureExtractor:
    def __init__(self, cache_dir="features", max_length=1024):
        ...
        # 动态批处理参数
        self.batch_sizes = {
            'cuda': self._auto_detect_batch_size(),
            'cpu': 8
        }
    ...
    def _get_cache_path(self, sequence):
        """获取缓存路径（使用两级目录减少单个目录文件数）"""
        hash_key = hashlib.md5(sequence.encode()).hexdigest()
        subdir = self.cache_dir / hash_key[:2]
        subdir.mkdir(exist_ok=True)
        return subdir / f"{hash_key}.npy"
    
    def _process_batch(self, batch_sequences):
        """处理单个批次"""
        inputs = self.tokenizer(
            [" ".join(seq) for seq in batch_sequences],
            return_tensors="pt",
            padding=True,
            truncation=True,
            max_length=self.max_length
    ...
    def extract_features(self, sequences, max_workers=8):
        """主提取函数"""
        batch_size = self.batch_sizes[self.device.type]
        total_sequences = len(sequences)
        
        # 进度跟踪
        progress = tqdm(total=total_sequences, desc="总进度")
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 第一阶段：检查缓存
            cache_tasks = {
                executor.submit(self._get_cache_path, seq): i
                for i, seq in enumerate(sequences)
            }
            
            cached_features = np.zeros((total_sequences, 1024), dtype=np.float32)
            uncached_indices = []
            path_mapping = {}
            
            for future in as_completed(cache_tasks):
                idx = cache_tasks[future]
                cache_path = future.result()
                path_mapping[idx] = cache_path
                
                if cache_path.exists():
                    cached_features[idx] = np.load(cache_path)
                else:
            ...
            # 第二阶段：处理未缓存序列
            if uncached_indices:
                print(f"需要处理 {len(uncached_indices)} 条新序列...")
                ...
                for batch_start in range(0, len(uncached_indices), batch_size):
                    ...
                    try:
                        features = self._process_batch(batch_sequences)
                        
                        # 并行保存
                        save_tasks = []
                        for i, feat in zip(batch_indices, features):
                            cache_path = path_mapping[i]
                            save_tasks.append(
                                executor.submit(
                                    np.save, 
                                    cache_path, 
                                    feat.astype(np.float16)  # 半精度存储
                                )
                            )
                            cached_features[i] = feat
                        
                        # 等待保存完成
                        for _ in as_completed(save_tasks):
                            progress.update(1)
                            
                    except Exception as e:
                        print(f"批处理失败: {e}")
                        # 记录失败索引以便重试
                        with open("failed_indices.txt", "a") as f:
...
# -------------------- 训练准备 --------------------
def multilabel_balanced_sampling(sequences, labels, mlb, target_samples_per_class=5000):
    """
    多标签平衡抽样：确保每个标签至少有一定数量的样本
    参数:
        sequences: 原始序列列表
        labels: 多标签编码后的numpy数组 (n_samples × n_classes)
        mlb: MultiLabelBinarizer对象
        target_samples_per_class: 每个标签的目标样本数（按最少标签调整）
    返回:
        平衡后的(sequences_balanced, labels_balanced)
    """
    # 统计每个标签的样本分布
    class_counts = np.sum(labels, axis=0)
    min_samples = min(class_counts)
    
    # 计算每个标签的目标样本数（不超过原始数量）
    target_samples = min(target_samples_per_class, min_samples)
    ...
    # 转换为列表并保持原始顺序
    balanced_indices = sorted(sampled_indices)
    return [sequences[i] for i in balanced_indices], labels[balanced_indices]
def prepare_dataloaders(sequences, labels, extractor ,balance_size, mlb, batch_size=32, test_size=0.2, val_size=0.1, max_length=512):
    """
    准备数据加载器
    返回：train_loader, val_loader, test_loader, num_classes
    """
    print("执行多标签平衡抽样...")
    sequences_balanced, labels_balanced = multilabel_balanced_sampling(sequences, labels, mlb=mlb,target_samples_per_class=balance_size)  # 如果不需要mlb可设为None
    
    num_samples = len(sequences_balanced)
    indices = np.arange(num_samples)

    # 先分出测试集
    X_temp_idx, X_test_idx, y_temp, y_test = train_test_split(
        indices, labels_balanced,
        test_size=test_size,
        random_state=42
    )
    ...
    # 预计算所有特征
    print("预计算特征...")
    all_features = extractor.extract_features(sequences_balanced,max_workers=8)
...
def main_dat(file_path, obo_file_path="go-basic.obo"):
    ...
    # 2. 读取原始数据
    #分类处理
    file_ext = os.path.splitext(file_path)[1].lower()  # 提取扩展名并转为小写
    ...
    if file_ext == ".csv":
       ...
       print("文本文件")
        ...
        print(f"从Excel文件加载原始数据: {file_path}...")
       ...
       print(f"从dat文件加载原始数据: {file_path}...")
       ...
       original_go_annotations_dict = mf_annotations
    ...
    mapped_annotations_dict = load_or_create_go_slim_map(original_go_annotations_dict)
    ...
    valid_protein_ids = set(sequences.keys()).intersection(set(mapped_annotations_dict.keys()))
    ...
    mlb1 = ids_ordered_by_mlb
    # 根据mlb处理后的ID顺序，准备最终的序列列表
    final_sequences_list = [sequences[pid] for pid in ids_ordered_by_mlb]
    
    # 初始化提取器
    extractor = ProtBERTFeatureExtractor(cache_dir="large_scale_features")
...
if __name__ == '__main__':
    ...
    uniprot_file = '/Users/mac/Desktop/学习相关/作业文档/医药人工智能/Project/uniprot_sprot.dat' # <--- 把你的文件名放在这里
    main_dat(uniprot_file)
